...
export async function PUT(request: NextRequest, props: any) {
  ...
  try {
    ...
    if (status === "APPROVED") {
      if (existingLaunch.type === "VOTO" && !session.user.canApproveVote) {
        return NextResponse.json({ error: "Sem permissão para aprovar votos" }, { status: 403 });
      }
      if (existingLaunch.type === "EBD" && !session.user.canApproveEbd) {
        return NextResponse.json({ error: "Sem permissão para aprovar EBD" }, { status: 403 });
      }
      if (existingLaunch.type === "CAMPANHA" && !session.user.canApproveCampaign) {
