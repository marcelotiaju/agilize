...
const navigation = [
  ...
  { name: 'Lançamentos', href: '/launches', icon: FileText },
  { name: 'Contribuintes', href: '/contributors', icon: Users },
  { name: 'Congregações', href: '/congregations', icon: Building },
  { name: 'Fornecedores', href: '/suppliers', icon: Building2 },
  { name: 'Classificações', href: '/classifications', icon: List },
  { name: 'Usuários', href: '/users', icon: UserCheck }, // Adicione esta linha
...
// {navigation.map((item) => {
//   // Verificar se o item requer permissão
//   if (item.permission && !session?.user?.[item.permission]) {
//     return null
//   }
    ...
    <>
      ...
      <div className="lg:hidden">
        <div className="flex items-center justify-between bg-white px-4 py-2 shadow">
          ...
          <h1 className="text-lg font-semibold">Lance Fácil</h1>
          <div></div> {/* Spacer */}
        </div>
      </div>
    </>
